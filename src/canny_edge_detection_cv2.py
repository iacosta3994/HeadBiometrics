...
def make_canny_magstripe(filename, filename_canny):                                # Function will load frame from file_path and will save canny edge image in same directory with new name
    ...
    clahe_img = clahe.apply(blurred_img)                                       # Adjsuts contrast in image to allow maggstripe to be darker for images with high gamma
    ret,thresh1 = cv2.threshold(clahe_img,50,255,cv2.THRESH_BINARY)              # Sets a threshold in search for the pixels near the color of black i.e magstripe
